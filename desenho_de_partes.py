...
coifatipo = int(input('Indique o tipo de coifa:\n1 - Cônica\n2 - Ogival Parabólica\n3 - Ogival\n\n'))
if coifatipo == 1:
	auxcoifatipo = 'cônica'
elif coifatipo == 2:
...
exibicao.append(f'Coifa {auxcoifatipo} de comprimento {coifacomprimento}cm, diâmetro {coifadiametro}cm, espessura de {coifaespessura}cm e densidade de {coifadensidade}Kg/m³.')
...
while True:
	...
	opcao = float(input(f'''Indique qual parte você deseja em seguida:\ncorpo (1)\ntransição (2)\nacréscimos (3)\naletas (4)\nfoguete completo (5)\n'''))
	...
	if opcao == 1:
		...
		exibicao.append('+')
		exibicao.append(f'Corpo de comprimento {corpocomprimento}cm, diâmetro {corpodiametro}cm, espessura {corpoespessura}cm e densidade de {corpodensidade}Kg/m³.')
		...
		transicaodiametrofinal = float(input('Insira o diâmetro final da transição (cm) '))

		exibicao.append('+')
		exibicao.append(f'Transição de comprimento {transicaocomprimento}cm, diâmetro inicial {transicaodiametroinicial}cm, diâmetro final {transicaodiametrofinal}cm, espessura {transicaoespessura}cm e densidade {transicaodensidade}Kg/m³.')
		...
		print('\n'*3,'#'*40,'\n'*3)
		opcaoacrescimo = float(input('''Indique o acrescimo desejado:
            ...
            '''))
		if opcaoacrescimo == 1:
			componentedemassadistancia = float(input('Insira a distância do componente de massa do nariz do foguete (cm) ')) 
			componentedemassamassa = float(input('Insira a massa do componente (Kg) '))
            
			exibicao.append('e')
			exibicao.append(f'Componente de massa de {componentedemassamassa}Kg a uma distância de {componentedemassadistancia}cm do nariz do foguete.')
			...
			exibicao.append('e')
			exibicao.append(f'Paraquedas de {massaparaquedas}Kg e CD de {CDparaquedas}.')
			...
			exibicao.append('e')
			exibicao.append(f'Motor de massa {motormassa}Kg, empuxo médio {motorempuxo}N, período de queima de {motorduracao}s e comprimento do motor {motorcomprimento}cm.')
		...
		aletacomprimentoraiz = float(input('Indique o comprimento da raiz de cada aleta (cm) '))
		aletacomprimentoponta = float(input('Indique o comprimento da ponta de cada aleta (cm) '))
		aletadesvio = float(input('Indique a distância vertical entre a raiz e a ponta de cada aleta (cm) '))
		...
		aletadistanciadofundo = float(input('Indique a distância do fundo do foguete de cada aleta (cm) '))
		aletadensidade = float(input('Indique a densidade das aletas (Kg/m³) '))

		exibicao.append('e')
		exibicao.append(f'Aletas de comprimento de raiz {aletacomprimentoraiz}cm, comprimento de ponta {aletacomprimentoponta}cm, desvio {aletadesvio}cm, largura {aletalargura}cm, espessura {aletaespessura}cm, quantidade {aletanumero}, distância do fundo {aletadistanciadofundo}cm e densidade {aletadensidade}Kg/m³')
		armazenamento += ['a1',aletacomprimentoraiz,'a2',aletacomprimentoponta,
                    'a3',aletadesvio,'a4',aletalargura,'a5',aletaespessura,
                    'a6',aletanumero,'a7',aletadistanciadofundo,
                    'a8',aletadensidade]
