...
def callCGcoifa (coifamassaM,coifamassam,coifacomprimento,coifacomprimentomenor):
	...
	CGcoifa = (coifacomprimento*2/3*coifamassaM - coifamassam*(coifacomprimento - coifacomprimentomenor/3))/(coifamassaM + coifamassam)
	return CGcoifa
...
def callCGcorpomotor (massamotor,massacorpo,corpocomprimento,motorcomprimento):
	...
	CGcorpomotor = (CGcorpo*massacorpo + CGmotor*massamotor)/(massacorpo + massamotor)
	return CGcorpomotor

def callCGaleta (param,aletamassa,aletadesvio,aletalargura,aletacomprimentoraiz,aletacomprimentoponta
	,aletadistanciadofundo,coifacomprimento,corpocomprimento):

	CGumaaleta = ((aletadesvio*aletalargura/2*param*(2*aletadesvio/3) + 
...
def callCGtransicao(transicaocomprimento, transicaodiametroinicial, transicaodiametrofinal, transicaodensidade):
    H = fabs(transicaocomprimento*transicaodiametroinicial/(transicaodiametroinicial - transicaodiametrofinal))
    h = fabs(transicaocomprimento*transicaodiametrofinal/(transicaodiametroinicial - transicaodiametrofinal))
...
def callCm (alpha,coifadiametro,A,d,foguetecomprimento,foguetevolume):
	try:
		Cm = -2*sin(alpha)/(A)*d*(foguetecomprimento*(pi*coifadiametro**2/4) - foguetevolume)
	except:
		Cm = 0
	return Cm
...
def callCDfriction (arearef,aletaareamolhada,Rcri,fitnessratio,corpoareamolhada,aletaespessura,reynoldsnumero,Rs,foguetecomprimento,Mach):
...
def callwind (altura):
    return 30*(4/1.225)**.5*(2.5)*log1p(1/2)*(altura)**0.25
